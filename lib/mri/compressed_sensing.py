...
def cartesian_mask(shape, acc, no_central_lines=10, centred=False):
    """
    ...
    if no_central_lines:
        pdf_x[Nx//2-no_central_lines//2:Nx//2+no_central_lines//2] = 0
        pdf_x /= np.sum(pdf_x)
        n_lines -= no_central_lines
    ...
    if no_central_lines:
        mask[:, Nx//2-no_central_lines//2:Nx//2+no_central_lines//2] = 1
    ...
    if not centred:
        mask = mri_math.ifftshift(mask, axes=(-1, -2))

    return mask


def shear_grid_mask(shape, acceleration_rate, sample_low_freq=True,
                    centred=False, no_central_lines=10):
    '''
...
def undersample(x, mask, centred=False, norm='ortho', noise=0):
    ...
    if centred:
        x_f = mri_math.fft2c(x, norm=norm)
        x_fu = mask * (x_f + nz)
        x_u = mri_math.ifft2c(x_fu, norm=norm)
        return x_u, x_fu
    else:
        x_f = mri_math.fft2(x, norm=norm)
        x_fu = mask * (x_f + nz)
        x_u = mri_math.ifft2(x_fu, norm=norm)
        return x_u, x_fu
...
def data_consistency(x, y, mask, centered=False, norm='ortho'):
    ...
    '''
    if centered:
        xf = mri_math.fft2c(x, norm=norm)
        xm = (1 - mask) * xf + y
        xd = mri_math.ifft2c(xm, norm=norm)
    else:
        xf = mri_math.fft2(x, norm=norm)
        xm = (1 - mask) * xf + y
        xd = mri_math.ifft2(xm, norm=norm)
