...
import torch
from torch.fft import ifftshift, fftshift, fft, fft2, ifft2, ifft
...
# TODO: All this needs to be converted from numpy to torch

def fftc(x, axis=-1, norm='ortho'):
    ''' expect x as m*n matrix '''
    return fftshift(fft(ifftshift(x, dim=axis), dim=axis, norm=norm), dim=axis)


def ifftc(x, axis=-1, norm='ortho'):
    ''' expect x as m*n matrix '''
    return fftshift(ifft(ifftshift(x, dim=axis), dim=axis, norm=norm), dim=axis)
...
def fft2c(x):
    ...
    res = fftshift(fft2(ifftshift(x, dim=axes), norm='ortho'), dim=axes)
    return res
...
def ifft2c(x):
    ...
    '''
    axes = (-2, -1)  # get last 2 axes
    res = fftshift(ifft2(ifftshift(x, dim=axes), norm='ortho'), dim=axes)
    return res
...
def fourier_matrix(rows, cols):
    ...
    col_range = torch.arange(cols)
    row_range = torch.arange(rows)
    scale = 1 / torch.sqrt(cols)

    coeffs = torch.outer(row_range, col_range)
    fourier_matrix = torch.exp(coeffs * (-2. * torch.pi * 1j / cols)) * scale
...
def inverse_fourier_matrix(rows, cols):
    ...
    return torch.conj(fourier_matrix(rows, cols))
